...
public class Timer {
    ...
    /**
     * Return the approximate match time. The FMS does not send an official match time to the
     * robots, but does send an approximate match time. The value will count down the time remaining
     * in the current period (auto or teleop). Warning: This is not an official time (so it cannot
     * be used to dispute ref calls or guarantee that a function will trigger before the match ends)
    ...
    /**
     * Pause the thread for a specified time. Pause the execution of the thread for a specified
     * period of time given in seconds. Motors will continue to run at their last assigned values,
     * and sensors will continue to update. Only the task containing the wait will pause until the
